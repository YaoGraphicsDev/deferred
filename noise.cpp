...
NoiseTexture::Grid NoiseTexture::strat_noise_2d_disk(uint32_t n_strata) {
    ...
    for (uint32_t ring = 0; ring < n_strata; ++ring) {
        ...
        for (uint32_t sector = 0; sector < n_strata; ++sector) {
            ...
            float sin = glm::clamp(sqrt(1 - cos * cos), -1.0f, 1.0f);
...
otcv::Image* NoiseTexture::disk_noise_texture(uint32_t tile_size, uint32_t n_strata_per_dim) {
 ...
 /*               float r1 = glm::length(noise_3d[layer * 2][row][col]);
               ...
               //float r1 = glm::length(noise_3d[layer * 2][row][col]);
               //float a1 = glm::degrees(glm::atan(noise_3d[layer * 2][row][col].y / r1, noise_3d[layer * 2][row][col].x / r1));
               //noise_data.push_back(r1);
               //noise_data.push_back(a1);
               //float r2 = glm::length(noise_3d[layer * 2 + 1][row][col]);
               //float a2 = glm::degrees(glm::atan(noise_3d[layer * 2 + 1][row][col].y / r2, noise_3d[layer * 2 + 1][row][col].x / r2));
               //noise_data.push_back(r2);
    ...
    // convert to unorm
    std::vector<uint8_t> noise_data_unorm(noise_data.size());
    for (uint32_t i = 0; i < noise_data.size(); ++i) {
        noise_data_unorm[i] = static_cast<uint8_t>(std::round(std::clamp(noise_data[i], 0.0f, 1.0f) * 255.0f));
    }
    ...
    texture = otcv::ImageBuilder()
        ...
        .build();
    texture->populate(noise_data_unorm.data(), noise_data_unorm.size() * sizeof(uint8_t), otcv::ResourceState::FragSample);
